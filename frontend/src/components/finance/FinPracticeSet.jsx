...
import { FIN_DIFFICULTY } from "@/data/finance.js";
...
function QuestionCard({ slug, q, index }) {
  ...
  const { getQuestionStatus, setQuestionStatus } = useFinanceProgress();
  const status = getQuestionStatus(slug, q.id);
  const diff = FIN_DIFFICULTY[q.difficulty] || FIN_DIFFICULTY.Medium;
    ...
    >
      <div className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {(q.answer?.length > 0 || q.keyPoints?.length > 0) && (
            <button
              ...
              style={
                showAnswer
                  ? { borderColor: "#14b8a6", color: "#14b8a6", backgroundColor: "#14b8a614" }
                  : { borderColor: "rgb(var(--border-strong))", color: "rgb(var(--text-muted))" }
              }
            >
        ...
        <Reveal open={showAnswer}>
          <div className="mt-3 space-y-3 rounded-lg border-l-2 surface-sunken p-4" style={{ borderColor: "#14b8a6" }}>
            {q.answer?.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide" style={{ color: "#14b8a6" }}>
                  <Compass className="h-3.5 w-3.5" /> How to think about it
                </div>
                {q.answer.map((p, i) => (
            ...
            {q.followUps?.length > 0 && (
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted">
                  <CornerDownRight className="h-3.5 w-3.5" /> Take it further
                </div>
                <ul className="space-y-1">
                  {q.followUps.map((p, i) => (
                    <li key={i} className="flex gap-2 text-sm text-muted">
...
export default function FinPracticeSet({ slug, questions }) {
  const { getQuestionStatus } = useFinanceProgress();
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl surface-sunken p-4">
        <p className="text-sm text-muted">
          Don't just read — answer each one in your own words first. Reveal the{" "}
          <span className="font-medium text-[color:rgb(var(--text))]">hint</span>, then the{" "}
          <span className="font-medium text-[color:rgb(var(--text))]">model answer</span> only after
          you've taken a real swing at it. That's how it sticks.
        </p>
