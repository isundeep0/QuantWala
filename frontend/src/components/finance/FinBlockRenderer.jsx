...
import FinDiagramView from "./FinDiagramView.jsx";
...
const CALLOUT = {
  ...
  rule: { icon: Target, color: "#14b8a6", label: "Rule of thumb" },
  myth: { icon: AlertTriangle, color: "#f59e0b", label: "Common myth" },
  reality: { icon: GraduationCap, color: "#0ea5e9", label: "Reality check" },
  win: { icon: Trophy, color: "#f59e0b", label: "The payoff" },
};
        ...
        {(block.items || []).map((s, i) => (
          <li key={i} className="flex gap-4 rounded-xl border p-4" style={{ borderColor: "rgb(var(--border))" }}>
            ...
            <div>
              ...
              {s.detail && <p className="mt-1 text-sm leading-relaxed text-muted">{formatInline(s.detail)}</p>}
            </div>
          </li>
        ))}
      </ol>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border p-4" style={{ borderColor: "#10b98144", backgroundColor: "#10b9810a" }}>
          <div className="flex items-center gap-2 text-sm font-semibold text-emerald-600 dark:text-emerald-400">
            <Check className="h-4 w-4" /> {block.prosLabel || "Pros"}
          </div>
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: "#ef444444", backgroundColor: "#ef44440a" }}>
          <div className="flex items-center gap-2 text-sm font-semibold text-red-600 dark:text-red-400">
            <X className="h-4 w-4" /> {block.consLabel || "Cons"}
          </div>
          <ul className="mt-2 space-y-1.5 text-sm text-muted">
            {(block.cons || []).map((p, i) => (
              <li key={i} className="flex gap-2">
...
function StatBlock({ block }) {
  const cols = block.items?.length === 2 ? "sm:grid-cols-2" : block.items?.length === 4 ? "sm:grid-cols-4" : "sm:grid-cols-3";
  return (
    <div>
      <BlockTitle>{block.title}</BlockTitle>
      <div className={`grid gap-3 ${cols}`}>
        {(block.items || []).map((s, i) => (
          <div key={i} className="rounded-xl surface-sunken p-4 text-center">
            <div className="text-2xl font-extrabold" style={{ color: s.color || ACCENT }}>
              {s.value}
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>
        <Sigma className="h-4 w-4" /> {block.title || "Formula"}
      </div>
      {block.expression && (
        <div
          className="mt-3 overflow-x-auto rounded-xl px-4 py-3 text-center font-mono text-base font-semibold"
          style={{ backgroundColor: "rgb(var(--bg-sunken) / 0.6)", color: "rgb(var(--text))" }}
        >
          {block.expression}
        </div>
      )}
      {block.where?.length > 0 && (
        <ul className="mt-3 space-y-1.5 text-sm text-muted">
          {block.where.map((w, i) => (
            <li key={i} className="flex gap-2">
              <span className="font-mono font-semibold text-[color:rgb(var(--text))]">{w.symbol}</span>
              <span>= {formatInline(w.meaning)}</span>
            </li>
          ))}
        </ul>
      )}
...
export default function FinBlockRenderer({ blocks = [] }) {
  return (
