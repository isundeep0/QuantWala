...
function NodeCard({ node }) {
  ...
  return (
    ...
    >
      ...
      >
        <SdIcon name={node.icon} className="h-4 w-4" />
      </span>
...
// Renders a left-to-right (default) or top-to-bottom data-flow diagram from a
// list of column "groups". Connectors animate subtly. Responsive: collapses to
// a vertical flow on small screens.
export default function DiagramView({ direction = "lr", groups = [], legend = [], caption }) {
          ...
          {groups.map((g, i) => (
            ...
            >
              ...
              {i < groups.length - 1 && (
                ...
                >
                  {/* horizontal arrow on desktop, vertical on mobile / TB */}
                  <ChevronRight
